...
public:
  ...
  static void
  PrintList(const std::pair<unsigned int, std::optional<unsigned int>> &node);
...
void TextReaderTest::PrintList(
    const std::pair<unsigned int, std::optional<unsigned int>> &node) {
  std::string dest =
      node.second.has_value() ? std::to_string(*(node.second)) : "";
